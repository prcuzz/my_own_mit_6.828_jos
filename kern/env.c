...
env_setup_vm(struct Env *e)
{
	...
	// memset(e->env_pgdir, 0, PDX(UTOP - 0) * sizeof(pde_t));
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (; start < end; start += PGSIZE)
	{
		...
		if (p == NULL)
			panic("[zzc]out of free pages\n");
		if (page_insert(e->env_pgdir, p, start, (PTE_U | PTE_W)) == -E_NO_MEM)
			panic("[zzc]out of memory\n");
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// LAB 3: Your code here.
	// struct PageInfo *p = page_alloc(0);
	// page_insert(e->env_pgdir, p, (void *)(USTACKTOP - PGSIZE), PTE_U);
